...
$TranscriptLogFile = Join-Path $LogFilePath "Transcript_ADDS.txt"
Start-Transcript -Path $TranscriptLogFile

$MainLogFile = Join-Path $LogFilePath "Mainlog.txt"
Add-Content -Path $MainLogFile -Value "------------------------------------------------------"
Add-Content -Path $MainLogFile -Value "$((Get-Date).ToString()): Install-ADDS.ps1 script started..."
Add-Content -Path $MainLogFile -Value "------------------------------------------------------"
...
#Create Scheduled Task to Install ADFS
...
if ([string]::IsNullOrEmpty($schedTask)) {
    ...
    Register-ADFSToolkitLabScheduledTask -Name $TaskName -Description "This script installes ADFS on startup" -Command "Install-ADFS.ps1" -AtStartup -DeployScript
    Add-Content -Path $MainLogFile -Value "$((Get-Date).ToString()): Done!"
}

#Remove Install ADDS Task
...
#Remove Scheduled Task Reboot 1
$TaskName = "Reboot 1"
$schedTask = Get-ScheduledTask -TaskName "ADFSToolkit Lab - $TaskName" -TaskPath "\ADFSToolkit Lab\" -ErrorAction SilentlyContinue
if (![string]::IsNullOrEmpty($schedTask)) {
    Add-Content -Path $MainLogFile -Value "$((Get-Date).ToString()): Removing scheduled task: 'ADFSToolkit Lab - $TaskName'"
...
#Install AD-DS
$SafeModeAdministratorPassword = [guid]::NewGuid().Guid
$DomainName = "ADFSTkLabAD"
$DomainDNS = "adfstoolkit.local"

#Can I check if ADDS is already run?
Add-Content -Path $MainLogFile -Value "$((Get-Date).ToString()): Installing AD DS Forest"
try {
    Install-ADDSForest -SkipPreChecks -DomainName $DomainDNS -SafeModeAdministratorPassword (ConvertTo-SecureString $SafeModeAdministratorPassword -AsPlainText -Force) -DomainNetBIOSName $DomainName -InstallDNS -Confirm:$false -ErrorAction Stop
    Add-Content -Path $MainLogFile -Value "$((Get-Date).ToString()): Done!"
}
catch {
    Add-Content -Path $MainLogFile -Value "$((Get-Date).ToString()): Error!"
