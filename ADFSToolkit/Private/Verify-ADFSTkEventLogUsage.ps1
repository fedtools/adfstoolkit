    ...
    try {
        ...
        if ( $Settings.configuration.Logging -ne $null -and $Settings.configuration.Logging.HasAttribute('useEventLog') -and $Settings.configuration.Logging.useEventLog.ToLower() -eq 'true' )
        {
            ...
            if ([System.Diagnostics.EventLog]::Exists('$MyLogName') -and [System.Diagnostics.EventLog]::SourceExists('$MySource') )
            {
                ...
                Write-EventLog -LogName $MyLogName -Source $MySource -EventId 1 -Message (Get-ADFSTkLanguageText logEventLogUsed)
                $EventLogEnabled=1
                ...
                else 
                {
                    ...
                    if ($MyLogName -and $MySource )
                    {
                        ...
                        Write-EventLog -LogName $MyLogName -Source $MySource -EventId 1 -Message (Get-ADFSTkLanguageText logEventLogCreated)
                        $EventLogEnabled=1
                    }
                    else 
                    {
                        Write-Error (Get-ADFSTkLanguageText logCouldNotCreateEventLog)
                        $EventLogEnabled=0
                    }
