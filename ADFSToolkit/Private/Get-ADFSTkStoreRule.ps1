function Get-ADFSTkStoreRule {
    ...
    $ADFSTkStore = Get-AdfsAttributeStore -Name ADFSTkStore
    
    if ([string]::IsNullOrEmpty($ADFSTkStore)) {
...
"@
        ...
        if (![string]::IsNullOrEmpty($ADFSTkStore)) {
            ...
            foreach ($attrib in $AttributesFromStore.Values) {
                ...
                @RuleName = "Transform $($attrib.name) through Custom Store"
                c:[Type == "$($attrib.type):beforetransform"]
                => add(store = "ADFSTkStore", 
                types = ("$($attrib.type)"), 
                query = ";$($attrib.transformvalue.adfstkstorefunction);{0}", param = "[ReplaceWithSPNameQualifier]", param = "$($ADFSTkStore.Configuration.IDPSALT)", param = c.Value);
                
"@
# c:[Type == "http://schemas.xmlsoap.org/ws/2005/05/identity/claims/displayname:beforetransform "]
# => add(store = "ADFSTKStore", types = ("http://schemas.xmlsoap.org/ws/2005/05/identity/claims/displayname "), query = ";toupper;{0}", param = "entity", param = "salt", param = c.Value);
                }
            }
        }
